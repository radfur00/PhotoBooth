...
var photoBooth = (function () {
    ...
    // Cheese
    public.cheese = function (photoId) {
        $('#counter').text('');
        $('.loading').text(L10N.cheese);
        public.takePic(photoId);
    }

    // take Picture
    public.takePic = function (photoId) {
        processing = true;
        setTimeout(function () {
            $('#counter').text('');
            $('.spinner').show();
            ...
            success: function (result) {
                if (result.error) {
                    ...
                    $.ajax({
                        ...
                        dataType: "json",
                        cache: false,
                        success: function (result_do_pic) {
                            public.renderPic(result_do_pic);
                        },
                        error: function (xhr, status, error) {
                            public.errorPic('final');
                        }
                    });
                } else {
                    $('.spinner').hide();
                    $('.loading').text('');
                    $('.loading').text(L10N.next);
                    public.countdown(countDown, $('#counter'), photoId+1);

                }
            },
            error: function (xhr, status, error) {
                public.errorPic('temp');
            }
        });
    }
    ...
    // Take Picture Button -Inject cb function
        ...
        if (target.hasClass('gallery')) {
            ...
            if (!processing) {
                ...
                loader.slideDown('slow', 'easeOutBounce', function () {
                   public.countdown(countDown, $('#counter'), 1);
                });
            }
        }
    });
    ...
    // Countdown Function
    public.countdown = function (calls, element, photoId) {
        count = 0;
        current = calls;
